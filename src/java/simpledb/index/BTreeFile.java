...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		//2. split the current page to the new page
		...
		//3. change the current page right brother pointer to change the right brother left pointer point to the new right page
		...
		//4. change the current page and new right page pointer
		...
		//5. change the page which point the current page
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//1.get the tuple number need to steal
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		int curEntriesNum = page.getNumEntries();
		int siblindEntriesNum = leftSibling.getNumEntries();
		int targetEntriesNum = (curEntriesNum+siblindEntriesNum)>>1;

		//2.steal the parent entry
		BTreeEntry entry = it.next();
		BTreeEntry mid = new BTreeEntry(parentEntry.getKey(),entry.getRightChild(),page.iterator().next().getLeftChild());
		page.insertEntry(mid);
		curEntriesNum++;

		//3.steal the left sibling entry to the current page
		...
		//4.steal the max key in left sibling to the parent
		leftSibling.deleteKeyAndRightChild(entry);
		parentEntry.setKey(entry.getKey());
		parent.updateEntry(parentEntry);

		//5.update page and parent pointer
		dirtypages.put(page.getId(),page);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//1.delete the right page and insert to left page
		...
		//2.change left page right pointer point to the right page right point(if exist)
		...
		//3.use setEmptyPage() to set the tag of right page in header to null
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//1.get the parent entry and delete from parent
		...
		//2.get the right entry and insert to the left page
		...
		//4.set right page empty
		setEmptyPage(tid,dirtypages,rightPage.pid.getPageNumber());

		//5.delete parent entry;
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
